...
public class ForgotPasswordController {
    ...
    void btnCheckEmailAction(ActionEvent event) {
        ...
        try {
            ResultSet result = CrudUtil.execute("SELECT Email FROM employees WHERE Name = ?", Name);
            if (result.next()) {
    ...
    void btnFPForgotPasswordAction(ActionEvent event) {
        ...
        try {
            CrudUtil.execute("UPDATE employees SET Password = ? WHERE Email = ?", newPassword, email);
    ...
    void btnGoToSignInPageAction(ActionEvent event) throws IOException {
        ...
        AnchorPane newPage = FXMLLoader.load(getClass().getResource("/view/Login.fxml"));
    ...
    private void sendOTPEmail(String recipientEmail, String otpCode) {
        String from = "kaushalyadilmi.av2003@gmail.com";
        ...
        Session session = Session.getDefaultInstance(props, new Authenticator() {
            protected javax.mail.PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication("kaushalyadilmi.av2003@gmail.com", "pgnv ddjc meac ooiv");
            }
        });

        try {
            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress("kaushalyadilmi.av2003@gmail.com"));
